...
saved_path = "./breast_data_0120/"                #保存路径
...
for i in range(len(annotations)):
    ...
    total_csv_annotations[key] = np.array([[annotations.loc[i, 'StartX'], annotations.loc[i, 'StartY'], annotations.loc[i, 'EndX'], annotations.loc[i, 'EndX'],
     'mass']])
    total_csv_annotations[key] = np.concatenate((total_csv_annotations[key],
                       np.array([[annotations.loc[i, 'nipple_StartX'], annotations.loc[i, 'nipple_StartY'], annotations.loc[i, 'nipple_EndX'],annotations.loc[i, 'nipple_EndY'], 'nipple']]))
                        ,axis=0)
    total_csv_annotations[key] = np.concatenate((total_csv_annotations[key],
                       np.array([[annotations.loc[i, 'lesion2Chest_StartX'], annotations.loc[i, 'lesion2Chest_StartY'], annotations.loc[i, 'lesion2Chest_EndX'],annotations.loc[i, 'lesion2Chest_EndY'], 'chest']]))
                        ,axis=0)
    # total_csv_annotations[key] = np.concatenate((total_csv_annotations[key],
    #                    np.array([[annotations.loc[i, 'lesion2Skin_StartX'], annotations.loc[i, 'lesion2Skin_StartY'], annotations.loc[i, 'lesion2Skin_EndX'],annotations.loc[i, 'lesion2Skin_EndY'], 'skin']]))
    #                     ,axis=0)
