...
from data.breast import Breast
...
class GMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        anno_pair, perm_mat, mass_pair_index = self.ds.get_pair(self.cls)
        ...
        ret_dict = {'Ps': [torch.Tensor(x) for x in [P1_gt, P2_gt]],
                    ...
                    'mass_pair_index': mass_pair_index,
        ...
        if imgs[0] is not None:
            ...
            # print(ret_dict['images'][0].size())    #1*256*256
            # print("use image")
        ...
        # print("len ", len(anno_pair[0]['feats']))
        feat1 = torch.Tensor(anno_pair[0]['feats'][0]).unsqueeze(2)
        for pair0_index in range(1, len(anno_pair[0]['feats'])):
            feat_temp = torch.Tensor(anno_pair[0]['feats'][pair0_index]).unsqueeze(2)
            # print("feat_temp.size()", feat_temp.size())
            feat1 = torch.cat((feat1, feat_temp), 2)
            # print("feat1.size()", feat1.size())
        # print("feat1_final.size()", feat1.size())


        # print("len ", len(anno_pair[1]['feats']))
        feat2 = torch.Tensor(anno_pair[1]['feats'][0]).unsqueeze(2)
        for pair1_index in range(1, len(anno_pair[1]['feats'])):
            feat_temp_2 = torch.Tensor(anno_pair[1]['feats'][pair1_index]).unsqueeze(2)
            # print("feat_temp_2.size()", feat_temp_2.size())
            feat2 = torch.cat((feat2, feat_temp_2), 2)
            # print("feat2.size()", feat2.size())
        # print("feat2_final.size()", feat2.size())



        ret_dict['features'] = [feat1, feat2]
        # print("features", len(ret_dict['features']))
        # print("features", ret_dict['features'][0].size())            #1*1000*3/4
        # print("use feat")
